...
public sealed partial class HumanoidProfileEditor
{
        ...
        foreach (var department in departments)
        {
            ...
            var jobs = department.Roles
                .Select(jobId =>
                {
                    try
                    {
                        return _prototypeManager.Index(jobId);
                    }
                    catch (Exception e)
                    {
                        _sawmill.Error($"Failed to index job '{jobId}': {e}");
                        return null;
                    }
                })
            ...
            foreach (var job in jobs)
            {
                try
                {
                    ...
                    var jobIconPrototype = _prototypeManager.Index(job.Icon);
                    icon.Texture = _sprite.Frame0(jobIconPrototype.Icon);
                ...
                {
                    _sawmill.Error($"Failed to refresh job '{job.ID}': {e}");
                }
            }
        }
    ...
    public void RefreshAntags()
    {
        ...
        foreach (var antag in _prototypeManager.EnumeratePrototypes<AntagPrototype>().OrderBy(a => Loc.GetString(a.Name)))
        {
            ...
            {
                _sawmill.Error($"Failed to refresh antag '{antag.ID}': {e}");
            }
        }
    }
}
